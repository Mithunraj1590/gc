...
export default function ImpactWidget() {
    ...
    useEffect(() => {
        ...
        function initSprites() {
            ...
            // Inactive Sprite
            inactiveSprite = document.createElement('canvas');
            inactiveSprite.width = size;
            inactiveSprite.height = size;
            const iCtx = inactiveSprite.getContext('2d')!;
            iCtx.fillStyle = 'rgba(200,200,200,0.08)';
            iCtx.beginPath();
            iCtx.arc(center, center, r * 0.3, 0, Math.PI * 2);
            iCtx.fill();
        }
        ...
        function isInsideShape(shapeIndex: number, dx: number, dy: number, R: number, time: number) {
            ...
            if (s === 0) {
                ...
                const dEyeL2 = (dx + eX) * (dx + eX) + (dy - eY) * (dy - eY);
                if (dEyeL2 < eOR2) {
                ...
                if (idleAnimIndex === 0 || idleAnimIndex === 4 || scrollFraction > 0.05) {
                    if (dy > R * 0.2 && d2 < R2 * 0.49 && d2 > R2 * 0.25) return false;
                } else if (idleAnimIndex === 5) {
                    if (dy > R * 0.15 && dx > -R * 0.5 && dx < R * 0.5) {
                        if (dy > dx * 0.5 + R * 0.2 && dy > -dx * 0.5 + R * 0.2 && d2 < R2 * 0.49) return false;
                    }
        ...
        function draw() {
            ...
            for (let i = 0; i < dotCount; i++) {
                ...
                if (swirlStrength > 0.01) {
                    ...
                    const dx_tw = tx - cx;
                    const dy_tw = ty - cy;
                    tx = cx + dx_tw * Math.cos(twistAngle) - dy_tw * Math.sin(twistAngle);
                    ty = cy + dx_tw * Math.sin(twistAngle) + dy_tw * Math.cos(twistAngle);
                }
